...
import { ArchivosCoasService } from '../servicios/coas/archivoscoas.service';
...
export class CertificacionesComponent implements OnInit {
  ...
  constructor(private datosService: DatosService, private contingenciaService: ContingenciaService, private matrizService: MatrizcuentasService, private pagosService: PagosService, private httpClient: HttpClient, private archivosCoasService: ArchivosCoasService) { }
 ...
 next : (data) =>{
  ...
  onFileChange(event: any) {
    console.log("tipo de archivo: " + this.catTipArch);
  ...
  submit() {
    //console.log(this.catBases);
    //console.log(this.id_Data, this.ip_Data, this.port_Data, this.name_Data);
    Swal.fire({
    ...
    let idConexion = '{"idBase":' + this.catBases + ', "tipArch":' + this.catTipArch + '}'
    this.archivosCoasService.dataMasiva(idConexion, this.myFiles)
      .subscribe(
        data => {
          //console.log(data)
        },
  ...
  /*async processFilesIni(): Promise<void> {
    this.message = [];

    if (this.selectedFiles) {
      for (let i = 0; i < this.selectedFiles.length; i++) {
        await this.processFile(this.selectedFiles[i]);
      }
    }
  }
  ...
  async processFile(file: File) {
    if(file){
      console.log("processFile Component");
    ...
    Swal.fire({
     ...
     //const file: File | null = this.selectedFiles.item(0);
      this.currentFile = file
      this.archivosCoasService.dataMasiva(this.currentFile).subscribe(
        data => {
          console.log(data)
        },
        err => {
